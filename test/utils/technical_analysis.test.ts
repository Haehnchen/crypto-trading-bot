...
import * as ta from '../../src/utils/technical_analysis';

describe('#technical_analysis for candles', () => {
  function createCandleFixtures(): ta.Candlestick[] {
  ...
  it('technical_analysis for options are created', async () => {
    ...
    assert.equal(
      1.12,
      parseFloat((result.macd_ext_dema_slow[result.macd_ext_dema_slow.length - 1] as any).histogram).toFixed(2)
    );

    assert.equal(result.bb.length > 0, true);
    assert.equal((result.bb[0] as any).lower < (result.bb[0] as any).middle, true);
    assert.equal((result.bb[0] as any).middle < (result.bb[0] as any).upper, true);
    assert.equal((result.bb[0] as any).lower < (result.bb[0] as any).upper, true);
    assert.equal((result.bb[0] as any).width > 0, true);

    assert.equal(result.bb_talib.length > 0, true);
    assert.equal((result.bb_talib[0] as any).lower < (result.bb[0] as any).middle, true);
    assert.equal((result.bb_talib[0] as any).middle < (result.bb[0] as any).upper, true);
    assert.equal((result.bb_talib[0] as any).lower < (result.bb[0] as any).upper, true);
    assert.equal((result.bb_talib[0] as any).width > 0, true);

    assert.equal((result.stoch[0] as any).stoch_k > 0, true);
    assert.equal((result.stoch[0] as any).stoch_d > 0, true);

    assert.equal((result.stoch_rsi[5] as any).stoch_k > 0, true);
    assert.equal((result.stoch_rsi[5] as any).stoch_d > 0, true);

    assert.equal(result.pivot_points_high_low.filter((v: any) => 'high' in v && 'close' in v.high).length > 2, true);
    assert.equal(result.pivot_points_high_low.filter((v: any) => 'low' in v && 'close' in v.low).length > 2, true);

    assert.equal(
      result.pivot_points_high_low.filter((v: any) => 'high' in v && 'close' in v.high).length >
        result.pivot_points_high_low_2.filter((v: any) => 'high' in v && 'close' in v.high).length,
      true
    );
    ...
    const volumeByPrice = (result.volume_by_price as any)[0][0];

    assert.equal(volumeByPrice.low > 0, true);
    assert.equal(volumeByPrice.high > 0, true);
    assert.equal(volumeByPrice.volume > 0, true);
    assert.equal((result.volume_by_price as any)[0].length, 12);

    assert.equal((result.zigzag as any[]).filter((v: any) => v.turningPoint === true).length > 0, true);

    assert.equal(result.volume_profile.length, 14);

    const ichimokuCloud = Object.keys((result.ichimoku_cloud as any)[0]);
  ...
  it('technical_analysis for pivot points with wicks for a range', () => {
    const highs = [1, 2, 3, 4, 5, 4, 3, 2, 1].map(v => {
      return { close: v, high: v * 1.2, low: v * 0.8, time: 0, open: 0, volume: 0 };
    });
